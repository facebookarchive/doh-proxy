...
def save_to_store(f):
    @wraps(f)
    def call(*args, **kw):
        args[0]._result_store[f.__name__] = (
            args[1:],
            kw,
        )
        return f(*args, **kw)
...
class Client(client_protocol.StubServerProtocol):
    ...
    def _on_start_request_bad_body(self, client, headers, end_stream):
        headers = modify_headers(headers, "content-length", str(len(b"badcontent")))
    ...
    def _on_start_request_bad_dns(self, client, headers, end_stream):
        headers = modify_headers(headers, ":path", self._make_get_path(b"badcontent"))
        return super().on_start_request(client, headers, end_stream)
...
class ServerIntegrationBaseClassTest(object):
    ...
    def setUp(self):
        self.logger = utils.configure_logger("doh-integration", level=self.ARGS.level)
    ...
    async def test_identical_id_on_reply(self):
        ...
        orig_qid = q.id
        await self.client.make_request(None, q)
        addr, msg = self.client.from_store("on_answer")[0]
        r = dns.message.from_wire(msg)
        self.assertEqual(orig_qid, r.id, "Query and Response IDs are not matching.")
    ...
    async def test_not_implemented_method(self):
        ...
        """
        with patch(
            "__main__.Client.on_start_request", new=Client._on_start_request_bad_method
        ):
            q = dns.message.make_query(qname="www.example.com", rdtype="A")
    ...
    async def test_bad_data(self):
        ...
        if self.ARGS.post:
            with patch(
                "__main__.Client.on_send_data", new=Client._on_post_bad_body
            ), patch(
            ...
            with patch(
                "__main__.Client.on_start_request", new=Client._on_start_request_bad_dns
            ):
                q = dns.message.make_query(qname="www.example.com", rdtype="A")
...
class ServerIntegrationPostTest(ServerIntegrationBaseClassTest, unittest.TestCase):
    def setUp(self):
        ServerIntegrationBaseClassTest.ARGS.post = True
        super().setUp()


class ServerIntegrationGetTest(ServerIntegrationBaseClassTest, unittest.TestCase):
    def setUp(self):
    ...
    async def test_head_method(self):
        ...
        """
        with patch(
            "__main__.Client.on_start_request", new=Client._on_start_request_head_method
        ):
            q = dns.message.make_query(qname="www.example.com", rdtype="A")
...
def main():
    ...
    unittest.main(argv=[sys.argv[0]] + args.args, testRunner=DOHTestRunner, verbosity=2)
