...
def echo_dns_q(q):
    return aiohttp.web.Response(
        status=200, body=q.to_wire(), content_type=constants.DOH_MEDIA_TYPE,
    )
...
class HTTPProxyPOSTTestCase(HTTPProxyTestCase):
    ...
    async def test_post_request_no_content_type(self):
        ...
        """
        request = await self.client.request(
            self.method, self.endpoint, headers={}, data=self.make_body(self.dnsq)
        )
    ...
    async def test_post_request_empty_body(self):
        ...
        """
        request = await self.client.request(
            self.method, self.endpoint, headers=self.make_header(),
        )
    ...
    async def test_post_request_bad_dns_request(self):
        ...
        """
        request = await self.client.request(
            self.method, self.endpoint, headers=self.make_header(), data="dummy",
        )
...
class DNSClientLoggerTestCase(HTTPProxyTestCase):
    ...
    @unittest_run_loop
    async def test_mock_dnsclient_assigned_logger(
        self, MockedDNSClient, Mockedon_answer, Mockedquery
    ):
        ...
        MockedDNSClient.assert_called_with(
            app.upstream_resolver, app.upstream_port, logger=mylogger
        )
